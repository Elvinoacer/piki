...
interface TripState {
  ...
  riderLocation: RiderPosition | null;
}
...
interface TripActions {
  ...
  // Real-time events
  setSearching: (isSearching: boolean, radiusKm?: number, ridersFound?: number) => void;
  updateStatus: (status: any, data?: { riderId?: string }) => void;
  ...
  devtools(
    (set, get) => ({
      ...
      setActiveTripId: (activeTripId) =>
        set(
          { activeTripId, step: "MATCHING", activeStatus: "REQUESTED" },
          false,
          "setActiveTripId"
        ),
      ...
      setSearching: (isSearching, searchRadiusKm = 0, ridersFound = 0) =>
        set({ isSearching, searchRadiusKm, ridersFound }, false, "setSearching"),

      updateStatus: (status, data) =>
        set((state) => ({
          activeStatus: status,
          assignedRiderId: data?.riderId ?? state.assignedRiderId,
        }), false, "updateStatus"),

      updateRiderPosition: (riderLocation) =>
        set({ riderLocation }, false, "updateRiderPosition"),
