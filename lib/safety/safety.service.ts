...
export async function getTripSharePublicView(token: string) {
  const link = await prisma.tripShareLink.findUnique({
    ...
    include: {
      trip: {
        include: {
          rider: {
            include: { riderProfile: true, riderLocation: true },
          },
        },
      },
    },
  ...
  return {
    tripId: trip.id,
    status: trip.status,
    riderName: trip.rider ? `${trip.rider.firstName ?? ""} ${trip.rider.lastName ?? ""}`.trim() || "Rider" : "Rider",
    riderPhoto: profile?.profilePhotoUrl ?? null,
    riderPlate: profile?.numberPlate ?? "",
    riderRating: profile?.ratingAverage ?? 0,
    pickup: {
      lat: trip.pickupLat,
      lng: trip.pickupLng,
    ...
    riderLat: trip.rider?.riderLocation?.latitude ?? undefined,
    riderLng: trip.rider?.riderLocation?.longitude ?? undefined,
  };
}
...
export async function getRiderIdentityBadge(riderId: string) {
  ...
  const badges: string[] = [];
  if (profile.verifiedAt) badges.push("Verified");
  if (profile.ratingAverage >= 4.8) badges.push("Top Rated");
...
async function recomputeUserRating(userId: string) {
  ...
  // Note: ClientProfile currently doesn't store a rating aggregate in the 3.1 minimal schema.
